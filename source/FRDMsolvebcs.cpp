...
static double LNPairingStrength   (double, double, LNParameter *);
...
void FRDMSoleveBCS(MFTSystem *sys, double rhobar, LNParameter *bcs, SPEnergy *spe)
{
  ...
  if(calcmonitor){
    cout << setw(12) << bcs->strength;
    cout << setw(12) << bcs->chemical_potential;
    cout << setw(12) << bcs->pairing_gap;
    cout << setw(12) << bcs->lambda2;
    cout << endl;
  }
#endif
}
...
void LNSolveBCS(LNParameter *bcs, SPEnergy *spe)
{
  ...
  for(k=0 ; k<kmax ; k++){
    ...
    if(calcmonitor){
      cout << setw(6) << k;
      cout << setw(12) << y[0] << setw(12) << y[1];
      cout << setw(12) << bcs->chemical_potential + p0[0];
      cout << setw(12) << bcs->pairing_gap + p0[1];
      cout << setw(12) << bcs->lambda2;
      cout << setw(12) << damping << setw(12) << x0 << endl;
    }
    ...
    if(p1[1] + bcs->pairing_gap < 0.0) p1[1] = abs(p1[1] + bcs->pairing_gap) -  + bcs->pairing_gap;
  ...
  if(k == kmax) cerr << "WARNNING : Lipkin-Nogami didnt converge" << endl;
...
/*      Pairing Strength G                                */
/**********************************************************/
double LNPairingStrength(double rhobar, double deltag, LNParameter *bcs)
{
...
void LNSolutionCheck(LNParameter *bcs, SPEnergy *spe)
{
  ...
  for(int i = 1 ; i<= div ; i++){
    ...
    for(int j = 1 ; j<= div ; j++){
      ...
      cout << p0[0] + bcs->chemical_potential <<" " << p0[1] + bcs->pairing_gap << " ";
      cout << bcs->lambda2 << " ";
      cout << y[0]/d[0] << " " << y[1]/d[1] << " " << x0 << endl;
    }
    cout << endl;
  }
  return;
