...
/**********************************************************/
void macsStoreCrossSection(int ie, int nc)
{
  char   s[N_REACTION_RATE] = "0000000";
  string str;
...
/**********************************************************/
void macsReactionRate(int tid, Particle pin, double *t)
{
  double *g;
  ...
  if(pin == gammaray){
    cout << "# Ecms[MeV]  (g,g')[mb]   (g,n)[mb] ";
    cout << "  (g,p)[mb]   (g,a)[mb]  (g,2n)[mb]  (g,np)[mb]  (g,na)[mb]   (g,f)[mb] " << endl;

    for(int i=0 ; i<N_MACS_GGRID ; i++){
      cout << setw(12) << macs_ggrid[i];
      for(int j=0 ; j<N_REACTION_RATE ; j++) cout << setw(12) << lowfilter(crx.macs[j][i]);
      cout << endl;
    }
    cout << endl;
  }
  else{
    cout << "# Ecms[MeV]    (n,g)[mb]  (n,n')[mb]";
    cout << "   (n,p)[mb]   (n,a)[mb]  (n,2n)[mb]  (n,3n)[mb]  (n,4n)[mb]  (n,np)[mb]  (n,na)[mb]   (n,f)[mb]";
    cout << "   total[mb] elastic[mb]" << endl;

    for(int i=0 ; i<N_MACS_EGRID ; i++){
      cout << setw(12) << macs_egrid[i];
      for(int j=0 ; j<N_REACTION_RATE ; j++) cout << setw(12) << lowfilter(crx.macs[j][i]);
      cout << endl;
    }
    cout << endl;
  ...
  cout << "# Maxwellian Average Cross Sections" << endl;

  if(pin == gammaray){
    cout << "#   E[MeV]       T9[K]     PartFunc   (g,g')[mb]   (g,n)[mb]";
    cout << "   (g,p)[mb]   (g,a)[mb]  (g,2n)[mb]  (g,np)[mb]  (g,na)[mb]   (g,f)[mb]" << endl;
  }
  else{
    cout << "#   E[MeV]       T9[K]     PartFunc    (n,g)[mb]  (n,n')[mb]";
    cout << "   (n,p)[mb]   (n,a)[mb]  (n,2n)[mb]  (n,3n)[mb]  (n,4n)[mb]  (n,np)[mb]  (n,na)[mb]   (n,f)[mb]";
    cout << "   total[mb] elastic[mb]" << endl;
  }
