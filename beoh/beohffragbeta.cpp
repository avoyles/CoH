...
static string DEFAULT_DECAY_FILE = "decaybranchENDF.dat";
...
static Isotope      *nuk = NULL;  // properties of each nuclide
static unsigned int *idx = NULL;  // idx[MZA] = sequential number in the data file

static int  cfyCopyPromptYield  (void);
static int  cfyReadDecayData    (string, int, double);
...
/*      Decay                                             */
/**********************************************************/
void beohFragmentBetaDecay(double long_lived, string file)
{
...
/**********************************************************/
int cfyReadDecayData(string decaydata, int nuktotal, double long_lived)
{
  ifstream fp;
  string   line, s;
...
void cfyCheckData(const int nuktotal)
{
  ...
  cout << "#  K0  Z0  A0  M0  Initial     Final       T1/2     ";
  cout << "  NBR  K1  Z1  A1  M1  BRatio" << endl;
  for(int k=0 ; k<nuktotal ; k++){

    double t2 = (nuk[k].getLambda() == 0.0) ? 0.0 : log(2.0)/nuk[k].getLambda();

    cout << setw( 5) << k;
    cout << setw( 4) << nuk[k].getZ() << setw(4) << nuk[k].getA() << setw(4) << nuk[k].getM();
    cout << setw(12) << setprecision(4) << nuk[k].initial;
    cout << setw(12) << setprecision(4) << nuk[k].yield;
    cout << setw(12) << setprecision(4) << t2;
    cout << setw( 4) << nuk[k].mode.getNDM() << endl;

    for(int i=0 ; i<nuk[k].mode.getNDM(); i++){
      cout << "                                                         ";
      int k1 = nuk[k].mode.getNext(i);
      cout << setw(4) << k1;
      if(k1 < 0) cout << " decay daughter nuclide not found" << endl;
      else{
        cout << setw(4) << nuk[k1].getZ() <<  setw(4) << nuk[k1].getA() << setw(4) << nuk[k1].getM();
        cout << setw(12) << setprecision(4) << nuk[k].mode.getBranch(i) << endl;
      }
    }
  }
}
