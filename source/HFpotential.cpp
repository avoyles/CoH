...
static void HFInitialPotentialWS (int, Basis *, OneBodyPotential *, LaguerrePolynomial *, HermitePolynomial *);


/**********************************************************/
/*      Initial Potential                                 */
/**********************************************************/
void HFInitialPotential(bool frdm, MFTSystem *sys, Basis *basis, OneBodyPotential *pot, LaguerrePolynomial *fl, HermitePolynomial *fh)
{
...
/**********************************************************/
void HFInitialPotentialWS(int a, Basis *basis, OneBodyPotential *pot, LaguerrePolynomial *fl, HermitePolynomial *fh)
{
...
/*
  for(int j=-fh->nz ; j<=fh->nz ; j++){
    if(j == 0) continue;
    for(int i=0 ; i<fl->nr ; i++){
      cout << " " << setw(11) << sqrt(fl->x[i]/basis->bp2);
      cout << " " << setw(11) << fh->x[j]/basis->bz;
      cout << " " << setw(11) << pot[0].central[i][j];
      cout << " " << setw(11) << pot[0].spinorb[i][j];
      cout << " " << setw(11) << pot[1].central[i][j];
      cout << " " << setw(11) << pot[1].spinorb[i][j] << endl;
    }
    cout << endl;
  }
*/
...
/*
  for(int j=-fh->nz ; j<=fh->nz ; j++){
    if(j == 0) continue;
    for(int i=0 ; i<fl->nr ; i++){
      cout << setw(12) << fl->x[i] << setw(12) << fh->x[j];
      cout << setw(12) << pot[0]->central[i][j];
      cout << setw(12) << pot[0]->effmass[i][j];
      cout << setw(12) << pot[0]->spinorb[i][j] << endl;
    }
    cout << endl;
  }
*/
