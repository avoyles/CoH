...
static void BCSSingleParticleDensity (double, Basis *, SPEnergy *, Operator *, GridData *, LaguerrePolynomial *, HermitePolynomial *);
static void BCSMatrixElement (Basis *, double *, BCSParameter *, SPEnergy *, GridData *, GridData *, LaguerrePolynomial *, HermitePolynomial *);
static void BCSConstantMatrixElement (int, Basis *, double *, BCSParameter *);
static double BCSGapEquation (bool, int, double, Basis *, double *, SPEnergy *, double *);
static inline double BCSCutoffFactor1 (double, double);
static inline double BCSCutoffFactor2 (double, double);
static inline double BCSDenominator (double, double, double, double);
...
/**********************************************************/
void HFSolveBCS(int np, Basis *basis, BCSParameter *bcs, SPEnergy *spe,
                double *v2, double *matV, Operator *H, GridData *rho, GridData *spd,
                LaguerrePolynomial *fl, HermitePolynomial *fh)
{
  double *d0;
...
/**********************************************************/
void BCSSingleParticleDensity(double lambda, Basis *basis,
                              SPEnergy *spe, Operator *H, GridData *spd,
                              LaguerrePolynomial *fl, HermitePolynomial *fh)
{
    ...
    for(int i=0 ; i<fl->nr ; i++){
      for(int j=-fh->nz ; j<=fh->nz ; j++){ if(j == 0) continue;
        ...
        for(int k2 = 0 ; k2<basis->ndata[kb] ; k2++){
          ...
          if(abs(H[kb].vector[k1][k2]) < eps) continue;
...
/**********************************************************/
void BCSMatrixElement(Basis *basis, double *matV, BCSParameter *bcs, SPEnergy *spe,
                      GridData *rho, GridData *spd,
                      LaguerrePolynomial *fl, HermitePolynomial *fh)
{
...
/**********************************************************/
void BCSConstantMatrixElement(int np, Basis *basis, double *matV, BCSParameter *bcs)
{
  double v = - abs(bcs->strength)/(11.0 + (double)np);
  for(int k1=0 ; k1<basis->nv ; k1++){
...
/**********************************************************/
double BCSGapEquation(bool delta, int np, double lambda,
                      Basis *basis, double *matV, SPEnergy *spe, double *d0)
{
  ...
  for(int itr=0 ; itr<itermax ; itr++){
    ...
    for(int k2=0 ; k2<basis->nv ; k2++){
      ...
      if((spe->energy[k2] > truncate) && (abs(f2) < 1.0e-5)){
        lcut = k2; break;
      }
    }
    ...
    if( abs(lambda - x) < eps ) pass2 = true;
...
/*      Cut Off Factor                                    */
/**********************************************************/
static inline double BCSCutoffFactor1(double e, double lambda)
{
  const double cutenergy = 5.0;
  const double diffuse   = 0.5;

  double x = (abs(e - lambda) - cutenergy)/diffuse;
  double f = 1.0/sqrt(1.0 + exp(x));

  return(f);
}

static inline double BCSCutoffFactor2(double e, double lambda)
{
...
/**********************************************************/
static inline double BCSDenominator(double e, double lambda, double f, double d)
{
