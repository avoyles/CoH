...
{
  const string s = "[ Mean Field Theory Calculation ]";
  int n1 = s.length();
  int n2 = strlen(sec);
...
void MFTPrintNucleus(MFTSystem *s)
{
  ...
  cout << cline << "  AtomicNum    MassNum Radius[fm] Surf[fm2]  Vol[fm3]" << endl;
  cout << blank;
  outVal(11,s->getZ());
  outVal(11,s->getA());
  outVal(s->R0);
...
void MFTPrintExpansion(HFInterface *hfsp)
{
  ...
  for(int p=0 ; p<=1 ; p++){
    ...
    if(p == 0) cout << "# neutron shell" << endl;
    else       cout << "# proton shell" << endl;
    ...
    for(int i=0 ; i<hfsp[p].n ; i++){
      ...
      cout << "# Ebind      V2         U           Ns   K2 Lmax" << endl;
      ...
      if((hfsp[p].state[i].m)%5 != 0) cout << endl;
      cout << "#    Sum ";
      outVal(s); nl();
    }
    nl();
