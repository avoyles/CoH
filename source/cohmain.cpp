...
static void cohSuperLazy        (double, int, int);
static int  cohEntranceChannel  (void);
static void cohCheckRange       (ZAnumber *, double );
static inline int cohEGrid      (int, double *);
...
/*      Main Loop                                         */
/*      --------                                          */
/*      In each data block in the input, this is called.  */
/*      all data will be initialized.                     */
/**********************************************************/
void cohMainLoop(double elab, int targz, int targa, unsigned long nsim)
{
  ...
  do{
    ...
    if(sec == BEGIN){
      ...
      for(int i=0 ; i<MAX_EINCIDENT ; i++){ if(ein[i] == 0.0) break;
        ...
        /*** DWBA */
        if(ctl.dwba){
          int iret = dwbaCalc(sys.cms_energy,sys.excitation,&sys.incident,&sys.target,
                              sys.reduced_mass,&dir,&crx);
          if(iret < 0) break;
        }
...
/**********************************************************/
void cohSuperLazy(double elab, int targz, int targa)
{
...
int cohEntranceChannel()
{
  ...
  if(ctl.deformed){
    jmax = ccCalc(sys.cms_energy, sys.excitation, &sys.incident, &sys.target,
                  sys.reduced_mass, &dir, crx.transmission, &crx);
  }

  /*** spherical optical model */
  else
    jmax = omCalc(sys.cms_energy, &sys.incident, &sys.target, sys.reduced_mass,
                  crx.transmission[0], &crx);
...
/**********************************************************/
static inline int cohEGrid(int k, double *ein)
{
  int n = 0;
  double *e;
