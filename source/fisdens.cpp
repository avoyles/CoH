...
#ifdef FISDENS_COH
static double  fldDensityEnhancement           (int, double, ZAnumber *, LevelDensity *);
#endif
#ifdef FISDENS_GNASH
static double  fldDensityEnhancementGNASH      (double, double, double);
#endif
#ifdef FISDENS_CCONE
static double  fldFissionDensityCCONE          (int, double, double, ZAnumber *, LevelDensity *);
#endif

static double  fldHighestTransitionState       (Barrier *);
//static inline double fldVibrationalEnhancement (double, double);
static inline double fldRotationalEnhancement  (int, int, double, double);
static inline double fldSigmaParalell          (double, double, double, double);
static inline double fldSigmaPerpendicular     (double, double);
...
/**********************************************************/
double fldDensityEnhancement(int i, double ex, ZAnumber *za, LevelDensity *ldp)
{
...
/**********************************************************/
double fldRotationalEnhancement(int i, int n, double sperp, double spara)
{
  double krot = 1.0;
...
/**********************************************************/
double fldSigmaParalell(double b2, double a, double t, double mass)
{
...
/**********************************************************/
double fldSigmaPerpendicular(double b2, double s)
{
//b2 *= 1.0 / (1.0+exp( (t-2.0) / 0.5 ));
...
/**********************************************************/
#ifdef FISDENS_GNASH
double fldDensityEnhancementGNASH(double ex, double f, double pair)
{
  double ux = ex - pair;
  double ef = 1.0;
...
/**********************************************************/
#ifdef FISDENS_CCONE
double fldFissionDensityCCONE(int i, double j, double ex, ZAnumber *za, LevelDensity *ldp)
{
  /*** normal level density */
  double mass = (double)(za->getA());
  double eshell = 2.6;
