...
/*      Population Distributed Over the Spin States       */
/**********************************************************/
void beohPopinitFile(const string fname, Nucleus *n)
{
  double sd;
  double se = 0.0;
  double so = 0.0;
  ...
  while(getline(fp,line)){
    if(j >= MAX_J) break;
    istringstream ss(&line[1]);  // skip comment #
...
void beohPopCheck(Nucleus *n)
{
  ...
  for(int k=0 ; k<n->ncont ; k++){
    ...
    for(int j=0 ; j<=n->jmax ; j++) {
      s1 += n->pop[k][j].even;
      s3 += n->pop[k][j].even;
      if(j < jm) cout << setprecision(2) << setw(9) << n->pop[k][j].even;
    }
    cout << endl;
    ...
    for(int j=0 ; j<=n->jmax ; j++) {
      s1 += n->pop[k][j].odd;
      s3 += n->pop[k][j].odd;
      if(j < jm) cout << setprecision(2) << setw(9) << n->pop[k][j].odd;
    }
...
/*
  ...
  for(int k=0 ; k<n->ncont ; k++){
    for(int j=0 ; j<=n->jmax ; j++) {
      cout << setw(5) << j << setprecision(4) << setw(13) << n->excitation[k] << setw(13)<< n->pop[k][j].even + n->pop[k][j].odd << endl;
    }
    cout << endl;
  }
*/
...
void beohPopProfile(Nucleus *n)
{
  ...
  for(int k=0 ; k<n->ncont ; k++){
    ...
    cout << setw(7) << setprecision(3) << n->excitation[k];
    cout.setf(ios::scientific, ios::floatfield);
    cout << setprecision(4) << setw(11) << s2 << endl;
