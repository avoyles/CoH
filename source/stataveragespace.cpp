...
/**********************************************************/
double statCalculateD0(double sp, Nucleus *n0, Nucleus *n1)
{
  double d0 = 0.0, sd = 0.0, pd = 0.0;
  ...
  pd = ldParityDistributionEdepend(1,ex,fd);
  sd = (i0 == 0) ? ldSpinDistribution(       0.5,n0->ldp.spin_cutoff,n0->ldp.sigma0,fs):
                   ldSpinDistribution((i0+1)*0.5,n0->ldp.spin_cutoff,n0->ldp.sigma0,fs)
                 + ldSpinDistribution((i0-1)*0.5,n0->ldp.spin_cutoff,n0->ldp.sigma0,fs);
...
/**********************************************************/
void statAdjustD0(double sp, Nucleus *n0, Nucleus *n1)
{
  ...
  while(k < max_iter){
    ...
    if(abs(d0/d0exp - 1.0) < 1e-5) break;
...
/**********************************************************/
void    statDecayWidth(System *sys, Pdata *pdt,
