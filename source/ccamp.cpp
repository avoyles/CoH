...
{
  try{
    amp = new complex<double> ** [col->nlevel];

    int c0 = (int)(2*fabs(col->lev[0].spin));
    int m0 = 3*(c0 + 1);

    for(int i=0 ; i<col->nlevel ; i++){
      amp[i] = new complex<double> * [MAX_ANGDIST];

      int c1 = (int)(2*fabs(col->lev[i].spin));
      int m1 = 3*(c1 + 1);

      for(int j=0 ; j<MAX_ANGDIST ; j++){
        amp[i][j] = new complex<double> [m0*m1];
        for(int k=0 ; k<m0*m1 ; k++) amp[i][j][k] = complex<double>(0.0,0.0);
      }
    }
  }
  catch(bad_alloc &e){
...
/**********************************************************/
void ccScatteringAmplitude(int jj, int nmax, double *th,
                           Collective *col, CCdata *cdt, complex<double> *c)
{
  unsigned long idx;
  ...
  for(int n0=0 ; n0<nmax ; n0++){
    ...
    complex<double> siga(0.0,0.0);
    if(col->lev[k0].coulomb != 0.0)
      siga = omCoulombPhaseFactor(cdt[n0].chn.l,
                                  col->lev[k0].coulomb,col->lev[k0].coulomb_scat0);
    ...
    for(int m0=-is0 ; m0<=is0 ; m0+=2){     int m0k  = (m0 +is0)/2; // projectile m
      for(int mm0=-i0 ; mm0<=i0 ; mm0+=2){  int mm0k = (mm0+i0 )/2; // target M
        ...
        for(int n1=0 ; n1<nmax ; n1++){
          ...
          complex<double> sigb(0.0,0.0);
          complex<double> a(0.0,0.0);

          if(col->lev[k1].coulomb != 0.0){
            complex<double> sig(0.0,0.0);
            sigb = omCoulombPhaseFactor(cdt[n1].chn.l,
                                        col->lev[k1].coulomb,col->lev[k1].coulomb_scat0);
            sig = siga * sigb;
            a   = sig  * c[idc];
          }
          else
...
/**********************************************************/
double ccAngularDistribution(int k, int j, double *th, Collective *col)
{
  unsigned long idx;
  double cx[3];
  ...
  if((k == col->target_index) && (col->lev[k0].coulomb != 0.0)){
    f = omRutherfordAmplitude(th[j],col->lev[k0].wave_number,
                              col->lev[k0].coulomb,col->lev[k0].coulomb_scat0);
    cx[2] = abs_square(f);
  }
  ...
  for(int m0=-is0 ; m0<=is0 ; m0+=2){        int m0k  = (m0 +is0)/2;
    for(int mm0=-i0 ; mm0<=i0 ; mm0+=2){     int mm0k = (mm0+i0 )/2;
      for(int m1=-is1 ; m1<=is1 ; m1+=2){    int m1k  = (m1 +is1)/2;
        for(int mm1=-i1 ; mm1<=i1 ; mm1+=2){ int mm1k = (mm1+i1 )/2;
          idx = (unsigned int)(  (i1+1)*((is1+1)*((i0+1)*m0k+mm0k)+m1k)+mm1k  );
          complex<double> a = amp[k][j][idx] / wave;
