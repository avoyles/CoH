...
static int     specCNDecaySpherical     (const int, const int, const int, const int, double,
                                         ...
                                         double **, Spectra *);
static int     specCNDecayDeformed      (const int, const int, const int, const int, double,
...
/**********************************************************/
void    specCompoundElastic(System *sys, double renorm, Transmission *tin, Transmission **tc,
                            Transmission **td, double **tg, Direct *dir, Spectra *spc)
{
  const int c0 = 0;
  const int k0 = 0;
  ...
  if(ctl.ewtransform){
    ...
    for(int j0=spings ; j0<=ncl[c0].jmax*2+spings ; j0+=2){
      double c3 = c2 * (j0+1.0);
      int f = 0;
...
/**********************************************************/
int     specCNDecaySpherical(const int c0, const int k0,
                             const int j0, const int p0, double g,
                             Transmission *tin, Transmission **tc, Transmission **td,
                             double **tg, Spectra *spc)
{
  ...
  for(int lp0=0 ; lp0<=tin->lmax*2 ; lp0+=2){
    ...
    for(int sp0=spinproj ; sp0>=-spinproj ; sp0-=2){
      ...
      if( abs(jp0-spintarg) > j0 || j0 > (jp0+spintarg) ) continue;
...
/**********************************************************/
int     specCNDecayDeformed(const int c0, const int k0,
                            const int j0, const int p0, double g,
                            Transmission **tc, Transmission **td,
                            double **tg, Spectra *spc, int *bidx)
{
  ...
  complex<double> x1(0.0,0.0), x2(0.0,0.0), xp(0.0,0.0);
  ...
  for(int a=0 ; a<m ; a++){ if(gCdt[a].level != gCol.target_index) continue;
    ...
    for(int b=0 ; b<m ; b++){
      ...
      complex<double> u1(0.0,0.0), u2(0.0,0.0);
      ...
      for(int c=0 ; c<m ; c++){
        ...
        for(int d=0 ; d<m ; d++){ if(c == d) continue;
          ...
          xp = complex<double>(cos(th),sin(th));
