...
static void  coulombBarnett (const int, const double, const double, complex<double> *, complex<double> *);
static void  coulombFRatio  (const int, const double, const double, double *);
static void  coulombFrecur  (const int, const double, const double, double *, complex<double> *, complex<double> *);
static complex<double> coulombWRatio (const int, const double, const double);
static double coulombPowerSeries (const double, const double);
static void  coulombClosed  (const int, const double, const double, complex<double> *, complex<double> *);
...
{
  complex<double> *C0 = new complex<double>[MAX_L + 1]; // G + iF
  complex<double> *C1 = new complex<double>[MAX_L + 1]; // G'+ iF'

  /*** closed channel */
  if(rho < 0.0){
    coulombClosed(MAX_L, rho, eta, C0, C1);
  }
  /*** Barnett-Aoki algorithm */
  else{
    coulombBarnett(MAX_L, rho, eta, C0, C1);
  }
...
void coulombBarnett(int lmax, double rho, double eta, complex<double> *c0, complex<double> *c1)
{
  double fw[3];

  /*** F(lambda+1) / F(lambda) */
  coulombFRatio(lmax, eta, rho, fw);

  /*** Recurrance Formula for F(lambda)
       fw[0] = F1, fw[1] = F0, fw[2] = F'(0)/F(0) */
  coulombFrecur(lmax, eta, rho, fw, c0, c1);

  /*** ratio (G'+iF')/(G+iF) = p + iq for l=0 */
  complex<double> w = coulombWRatio(0, eta, rho);
...
/**********************************************************/
void coulombFRatio(const int lambda, const double eta, const double rho, double *fw)
{
  int l = lambda + 1;
  ...
  if(!conv){
    message << "continued fraction calculation for F(l+1)/F(l) didn't converge";
    cohTerminateCode("coulombFRatio");
  }
...
/**********************************************************/
void coulombFrecur(const int lambda, const double eta, const double rho, double *fw, complex<double> *c0, complex<double> *c1)
{
...
/**********************************************************/
complex<double> coulombWRatio(const int lambda, const double eta, const double rho)
{
...
/*      Y. Aoki note                                      */
/**********************************************************/
double coulombPowerSeries(const double eta, const double rho)
{
  double b1 = 2 * PI * eta;
  ...
  if(!conv){
    message << "power seriese expansion didn't converge";
    cohTerminateCode("coulombPowerSeries");
  }
...
/**********************************************************/
void coulombClosed(const int lmax, const double rhox, const double eta, complex<double> *C0, complex<double> *C1)
{
  double p1, p2, p3;
  double rho = abs(rhox);
