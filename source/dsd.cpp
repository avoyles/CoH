...
static void dsdCalc               (const int, CCdata *, Bstate *, complex<double> *, double *);
static void dsdCalcHF             (CCdata *, Bstate *, complex<double> *, double *, HFInterface *);
...
static inline complex<double> phasefactor(int);
...
static complex<double> *f1=NULL, *f2=NULL;
static double  sig[3];
...
(int           incid,         // index for incident channel
 double        energy,        // LAB incident energy
 ...
 double        beta2,         // deformation parameter for double-humped GDR
...
//     Memory Allocation
  ...
  try{
    ...
    f1             = new complex<double> [MAX_POINTS];
    f2             = new complex<double> [MAX_POINTS];
    dwave          = new complex<double> [MAX_POINTS*MAX_L*3];
...
/**********************************************************/
void    dsdCalc(const int nlev, CCdata *cdt, Bstate *bst,
                complex<double> *dwave, double *gspec)
{
...
/**********************************************************/
void    dsdCalcHF(CCdata *cdt, Bstate *bst,
                  complex<double> *dwave, double *gspec, HFInterface *hf)
{
...
/**********************************************************/
bool    dsdAmplitudeSpherical(int l2, int j2, double *x,
                              CCdata *cdt, complex<double> *wf, Bstate *bw)
{
  double  z;
  complex<double> tdir,tcol,tall;

  x[0] = x[1] = x[2] = 0.0;
  if( (z = z_coefficient(l2,j2,2*bw->l,bw->j2,1,2))==0.0 ) return(false);

  complex<double> rmtd = dsdRadialIntegral(ninteg,integwidth,f1,bw->wave,wf);
  complex<double> rmtc = dsdRadialIntegral(ninteg,integwidth,f2,bw->wave,wf);
  complex<double> cnu  = complex<double>(const30,0.0)/complex<double>(cdt->lev->energy-gdr0.getEnergy()-bw->bind,gdr0.getWidth()/2.0);
...
/**********************************************************/
bool    dsdAmplitudeDeformed(const int nlev, const int ip, int l2, int j2, double *x,
                             CCdata *cdt, complex<double> *wf, Bstate *bw)
{
  complex<double> tdir,tcol,tall,cnu,cnu0,cnu1,cnu2,p,w;

  x[0] = x[1] = x[2] = 0.0;

  double z1 = z_coefficient(l2,j2,2*bw[ip].l,bw[ip].j2,1,2)/sqrt(bw[ip].j2+1.0);

  complex<double> rmtd(0.0,0.0);
  ...
  tcol = complex<double>(0.0,0.0);
  for(int iq=0 ; iq<nlev ; iq++){

    double z2 = z_coefficient(l2,j2,2*bw[iq].l,bw[iq].j2,1,2)/sqrt(bw[iq].j2+1.0);
    if(z2 == 0.0) continue;

    complex<double> rmtc(0.0,0.0);
    rmtc = dsdRadialIntegral(ninteg,integwidth,f2,bw[iq].wave,wf);
    rmtc *= phasefactor(bw[iq].l);

    cnu1 =
    cnu2 = complex<double>(const31,0.0)
          /complex<double>(cdt->lev->energy-gdr1.getEnergy()-bw[iq].bind,gdr1.getWidth()/2.0);
    cnu0 = complex<double>(const30,0.0)
          /complex<double>(cdt->lev->energy-gdr0.getEnergy()-bw[iq].bind,gdr0.getWidth()/2.0);
...
/**********************************************************/
void    dsdSetConstant(Particle p, int targZ, int targA, Optical *omp,
                       CCdata *cdt, double v1)
{
  double const3;
...
/*     Add to Spectrum                                    */
/**********************************************************/
void   dsdSpecBroadening(double *ds, double *gs, int ne, double de, double w)
{
  if(w == 0.0){
...
/**********************************************************/
inline void dsdHOWaveFunction(double b, double d, Bstate *bst)
{
  double r,c,p,q;
  int n = bst->n;
  int l = bst->l;
...
/**********************************************************/
inline complex<double> phasefactor(int l)
{
  double preal, pimag;
  ...
  return( complex<double>(preal,pimag) );
}
