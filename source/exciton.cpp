...
/**********************************************************/
void  preqExcitonPopulation(int id, Transmission *tc, Transmission *td, double *spc)
{
  int i = ncl[0].cdt[id].next;
...
/**********************************************************/
void preqRenormalizePopulation(double f)
{
  for(int id=1 ; id<MAX_CHANNEL ; id++){
    if(!ncl[0].cdt[id].status) continue;
    int    i  = ncl[0].cdt[id].next;
...
//  cout << esig[k1] <<" " << ein <<" " << esig[k2] << "  :  ";
//  cout << emesh[n1] <<" " << eout <<" " << emesh[n2] << " :  " << sigma2 <<  endl;
