...
/**********************************************************/
void HFMultipoleMoment(Basis *basis, Moment *moment, GridData *rho, 
                       LaguerrePolynomial *fl, HermitePolynomial *fh)
{
  ...
  moment->zcm  = (abs(zcm) > eps) ? zcm : 0.0;
  moment->q20  = (abs(q20) > eps) ? q20 : 0.0;
  moment->q30  = (abs(q30) > eps) ? q30 : 0.0;
  moment->q40  = (abs(q40) > eps) ? q40 : 0.0;
  moment->qn   = (abs(qn ) > eps) ? qn  : 0.0;
  moment->zmin = zmin;
/*
  cout << zcm << endl;
  cout << q20 << endl;
...
/**********************************************************/
void HFRenormalizeLocalDensity(const int n, const int z,
     GridData *rho, GridData *tau, GridData *d2rho, GridData *divJ,
     LaguerrePolynomial *fl, HermitePolynomial *fh)
{
  for(int p=0 ; p<2 ; p++){
