...
//    cout << loopout << " " << k1<< " "<<j1<<" " <<k2<< " " << mnext.getEk() <<" " <<mnext.getEnergy() << endl;
...
//cout << m0.getEnergy() <<" " <<  m1.getEnergy() <<  " " << ecms << " " << np << " " << ic <<  endl;
...
void mcWriteHistory()
{
  ...
  if(PrintParent){
    ...
    for(int i=0 ; i<np ; i++){
      ...
      cout << setw(3)  << m.getSpin();
      cout << setw(1)  << ((m.getParity() == -1) ? '-' : '+');
      cout << setw(13) << m.getEnergy();
    }
    cout << endl;
  }
}
