...
#ifdef omLegendre
static void omLegendreCoefficient (int, double, CrossSection *, complex<double> *);
#endif

#ifdef NormalizedWavefunction
void omWaveFunction(const int, const double, const double, const double, complex<double>, LevelData *, Potential *, Wavefunc *);
#endif
...
//      Memory Allocation
  ...
  try{
    ...
    smat = new complex<double> [MAX_L*3];
  }
  catch(bad_alloc &e){
...
//      Scattering Amplitude
  ...
  if(prn.angdist && ctl.entrance){
    ...
    for(int i=0 ; i<MAX_ANGDIST ; i++){
      ...
      case triton   :
      case helion   :
        p[i] = omScatteringAmplitudeP(cdt.lmax,cx,crx->costh[i],w,
                                      lev.coulomb,lev.coulomb_scat0,smat);
        break;
      case alpha    :
        p[i] = omScatteringAmplitudeA(cdt.lmax,cx,crx->costh[i],w,
                                      lev.coulomb,lev.coulomb_scat0,smat);
        break;
      case deuteron :
        p[i] = omScatteringAmplitudeD(cdt.lmax,cx,crx->costh[i],w,
                                      lev.coulomb,lev.coulomb_scat0,smat);
        break;
      default       :
        break;
...
//      Output Cross Sections
  ...
  if(ctl.entrance){
    ...
    for(int l=0 ; l<LPartial ; l++){
      ...
      cout << setw(12) << parSigT[l];
      cout << setw(12) << parSigE[l];
      cout << setw(12) << parSigR[l];
    }
    cout << endl;
#endif
...
/**********************************************************/
#ifdef omLegendre
void omLegendreCoefficient(int lmax, double x, CrossSection *crx, complex<double> *smat)
{
  if(lmax == 0) return;
  ...
  for(int l=0 ; l<2*lmax ; l++)  cout << l<<" " << p[l] * 4*PI / (2*l+1) << endl;
  ...
  for(int i=0 ; i<MAX_ANGDIST ; i++){
    ...
    cout << setw(12) << crx->theta[i] << setw(12) << y << endl;
  }
...
#ifdef NormalizedWavefunction
void omWaveFunction(const int l, const double xs, const double xj, const double mu, complex<double>smat, LevelData *lev, Potential *pot, Wavefunc *wfn)
{
  if(l != 3) return;

  complex<double>c = omNormalizationFactor(l,pot->n_match-3,lev->coulomb,lev->coulomb_scat0,smat,wfn);
  ...
  cout << "#  L = " << l << "  J = " << setprecision(1) << setw(5) << xj;
  cout << "           S = " << setprecision(4) << setw(12) << smat.real() << setw(12) << smat.imag();
  cout << "        T = " << setprecision(4) << setw(12) << 1.0 - norm(smat) << endl;
  cout << "# R[fm]       Real        Imag        Norm      ";
  cout << " V            W           V+L+SO      W+SO" << endl;

  for(int ir=0 ; ir<pot->n_match ; ir++){
    complex<double> q = xl_term*pot->r2inv[ir] - pot->mean_field[ir] - so_term*pot->spin_orbit[ir];
    double r = pot->width*ir;

    cout << setprecision(4);
    cout << setw(12) << r;
    cout << setw(12) << wfn->internal[ir].real() << setw(12) <<wfn->internal[ir].imag();
    cout << setw(12) << norm(wfn->internal[ir]);

    cout << setw(12) << -pot->mean_field[ir].real() / x;
    cout << setw(12) << -pot->mean_field[ir].imag() / x;
    cout << setw(12) << q.real() / x;
    cout << setw(12) << q.imag() / x;

    double d = arg(smat);
    cout << setw(12) << 0.5*(sin(k*r) + abs(smat)*sin(k*r + d));
    cout << setw(12) << 0.5*(cos(k*r) - abs(smat)*cos(k*r + d));
...
/*
  double s = 0.0;
  for(int ir=0 ; ir<pot->n_match ; ir++){
    s += abs(wfn->internal[ir]) * pot->mean_field[ir].imag() * pot->width;
  }
  cout << setprecision(4);
  cout << setw(12) << lev->energy << setw(12) << 1.0 - norm(smat) << setw(12) << s << endl;
*/
}
#endif
