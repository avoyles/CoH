...
/**********************************************************/
void    specLevelDecay(int c0, Pdata *pdt, Nucleus *n0, 
                       Transmission **tc, Transmission **td, Spectra *spc)
{
  ...
  for(int k0=n0->ndisc-1 ; k0>=1 ; k0--){
    ...
    if(tp > 0.0){
      ...
      if(tstat[gammaray]) tg = specLevelTransisionGamma(sumall,k0,p0,j0,0.0,n0);
