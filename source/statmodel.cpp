...
( System        *sys,              // system parameters
  ...
  unsigned long  nsim)             // number of Monte Carlo simulations
{
...
//      Statistical Model Main Part
  ...
  if(ctl.deformed && sys->inc_id != gammaray){
    statReplaceTransmission(sys->target_level,dir->lev,&ncl[sys->target_id],
                            sys->bandidx,td[sys->target_id],crx.transmission);
  }

  /***  direct/semidirect nucleon capture */
  if(ctl.dsd){
    dsdDirectCaptureModel(sys->inc_id,sys->cms_energy,&sys->incident,
                          &sys->target,sys->reduced_mass,cap,sys->beta2,spc.cn[0]);
...
/**********************************************************/
void statReplaceTransmission(int targlev, Level *dir, Nucleus *n, int *bidx, Transmission *td, double **transinc)
{
