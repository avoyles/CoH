...
/**********************************************************/
void beohParameterSetting(System *sys, GDR *gdr, bool meta)
{
  /*** bin width */
...
/*
  cout << "# GT Sum " << sum << "  "<< gts->nstate << endl;
  for(int i=0 ; i<gts->nstate ; i++){
    cout << "#GT "
         << gts->br[i].getE() <<" " << gts->br[i].getR() << endl;
  }
  cout << "# ENSDF Sum " << sum << "  "<< ens->nstate << endl;
  for(int i=0 ; i<ens->nstate ; i++){
    cout << "#ENSDF "
         << ens->br[i].getE() <<" " << ens->br[i].getR() << endl;
  }
*/
}
