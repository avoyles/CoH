...
static int   eclMCSimulation (const int, int *, int **, double, double ****, 
                              unsigned long ***, unsigned long **, unsigned long *, EXSpectra *);
static void  eclMCHistory    (int, int, int *, int *,
...
/**********************************************************/
void eclMC(const int nmax, const int kmax, const int cmax, int *ktot, int **cidx,
           double ****ptbl, double **mtpl, 
           unsigned long nsim, double de, EXSpectra *dat)
{
...
#ifdef CALC_MONITOR
    if(t%10000000L==0) cerr << "+";
    else if(t%1000000L==0) cerr << "*";
#endif
...
/*************************************************/
int eclMCSimulation(const int cmax, int *ktot, int **cidx, double de,
                    double ****ptbl, unsigned long ***hist, unsigned long **npar,
                    unsigned long *nevt, EXSpectra *dat)
{
...
/*  History Array                                */
/*************************************************/
void eclMCHistory(int c0, int mc, int *iout, int *kout, 
                  unsigned long ***hist, unsigned long **npar, unsigned long *nevt, double de)
{
  ...
  if(PRINT_HISTORY){
    ...
    for(int i=0 ; i<mc ; i++){
      double ep = kout[i] * de;
      cout << setw(2) << pname[ iout[i] ] << setw(12) << ep;
    }
    if(fission_event) cout << setw(2) << pname[7] << setw(12) << 0.0;
...
/*  Tally Events                                 */
/*************************************************/
void eclMCTally(const int nmax, const int cmax, int *ktot, unsigned int tsim, double **mtpl,
                unsigned long ***hist, unsigned long **npar, unsigned long *nevt,
                EXSpectra *dat)
{
  for(int n=0 ; n<nmax ; n++){
