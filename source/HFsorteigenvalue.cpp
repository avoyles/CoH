...
/*      ptr : block number that includes the eigenvalu    */
/*      ord : original order inside the block             */
/**********************************************************/
void HFSortEigenvalue(Basis *b, double *spe, int *idx, int *ptr, int *ord)
{
  ...
  for(int i=0 ; i<b->nv ; i++){
    for(int k=0 ; k<b->nb ; k++){
      ...
      /*** find original Omega block and its order in the block */
