...
void beohReadInputFiles(System *sys, FFragData *fdt)
{
  ...
  string ya = datadir + "/" + PARAMETERSDIRECTORY + fdt->yafile;
  for (int nc=0 ; nc < fdt->getFissionChance() ; nc++){
    int ZAIDt = ZAID - nc;
    if (! fdt->yafile.empty()){
      found = false;
      ifstream fp(ya.c_str(),ios::in);
      string line;
      int ZAIDtemp;

      if (!fp.is_open()) cout << "Parameters are not what you think for YA!!" << endl;
  ...
  string tke = datadir + "/" + PARAMETERSDIRECTORY + fdt->ytkefile;
  for (int nc=0 ; nc < fdt->getFissionChance() ; nc++){
    int ZAIDt = ZAID - nc;
    if (! fdt->ytkefile.empty()) {
      found = false;
      ifstream fp(tke.c_str(),ios::in);
      string line;
      int ZAIDtemp;

      if (!fp.is_open()) cout << "Parameters are not what you think for TKE!!" << endl;
  ...
  string rta = datadir + "/" + PARAMETERSDIRECTORY + fdt->rtafile;
  for(int nc=0 ; nc < fdt->getFissionChance() ; nc++){
    // read RTA file, if it is given
    int ZAIDt = ZAID - nc;
    if (! fdt->rtafile.empty()) {
       ifstream fp(rta.c_str(),ios::in);
       string line;
       int ZAIDtemp;
       int nmax;
       found = false;

       if (!fp.is_open()) cout << "Parameters are not what you think for RTA!!" << endl;
