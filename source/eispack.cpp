...
int EISPACKHermiteDiag(const int n, complex<double> *a, double *w, complex<double> *z)
{
  ...
  for(int i=1 ; i<=n ; i++){
    ...
    for(int j=1 ; j<=n ; j++){
      int ij = (i-1)*n+j-1;
      z[ij] = complex<double>(zr[i][j],-zi[i][j]);
    }
  }
...
int EISPACK_tred2(int n, double **a, double *d, double *e)
{
  int l,k,j,i;
...
int EISPACK_tqli(int n, double *d, double *e, double **z)
{
  int m,l,iter,i,k;
