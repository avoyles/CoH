...
static Wavefunc         *wfn;
static complex<double>  *Smat, *Cmat, *work[3], **Vpot;
static double           *Vmat[MAX_LAMBDA];
static int               Jmin = 0, Jmax = 0, Lmax = 0, Jcut = 0;
static NuclearModel      nucmodel = spherical;

static void ccSetCoupledLevels     (double, double, Pdata *, Direct *);
static void ccSetCouplingPotential (int, double, Pdata *, ZAnumber *);
...
/**********************************************************/
int ccPreset(double energy, double excitation, Pdata *proj, ZAnumber *targ, double mu, Direct *dir)
{
...
//     Memory Allocation
  ...
  try{
    ...
    Smat = new complex<double> [MAX_CCMATRIX*MAX_CCMATRIX];
    Cmat = new complex<double> [MAX_CCMATRIX*MAX_CCMATRIX];
    Vpot = new complex<double> * [MAX_POINTS];
    for(int i=0 ; i<MAX_POINTS ; i++){
      Vpot[i] = new complex<double> [MAX_CCMATRIX*(MAX_CCMATRIX+1)/2];
    }
    for(int i=0 ; i<MAX_LAMBDA ; i++){
      Vmat[i] = new double [MAX_CCMATRIX*(MAX_CCMATRIX+1)/2];
    }
    for(int i=0 ; i<3 ; i++){
      work[i] = new complex<double> [MAX_CCMATRIX*MAX_CCMATRIX];
    }
...
/*      Copy Level Data                                   */
/**********************************************************/
void ccSetCoupledLevels(double energy, double excitation, Pdata *proj, Direct *dir)
{
...
/**********************************************************/
void ccSetCouplingPotential(int zzprod, double mu, Pdata *proj, ZAnumber *targ)
{
  Optical *omp;
...
int ccHermiteMatrix(const int jj, const int p)
{
  ...
  for(int i=0 ; i<m ; i++){
    for(int j=0 ; j<m ; j++) pmat[i*m+j] = complex<double>(0.0,0.0);
  }
  for(int i=0 ; i<m ; i++){ if(!gCdt[i].open) continue;
    for(int j=0 ; j<m ; j++){ if(!gCdt[j].open) continue;
      ...
      pmat[ij] = ((i==j) ? complex<double>(1.0,0.0) : complex<double>(0.0,0.0)) - w;
    }
  }
...
/**********************************************************/
#ifdef DEBUG_CC_MATRIX
void ccCheckMatrix(const int m, complex<double> *pmat)
{
  ...
  for(int i=0 ; i<m ; i++){
    for(int j=0 ; j<m ; j++){
      complex<double> w = complex<double>(0.0,0.0);
      for(int k=0 ; k<m ; k++){
        int ik = i*m+k;
        int jk = j*m+k;
        w += gHermiteEigenvector[ik]*conj(gHermiteEigenvector[jk]);
      }
      if(abs(w.real())<1e-15) w.real(0.0);
      if(abs(w.imag())<1e-15) w.imag(0.0);
      cout << setw(14) << w.real() << setw(14) << w.imag();
    }
    cout << endl;
  }

  complex<double> *wmat = work[2];
  for(int i=0 ; i<m ; i++){
    for(int j=0 ; j<m ; j++){
      wmat[i*m+j] = complex <double>(0.0,0.0);
      for(int l=0 ; l<m ; l++){
        complex <double>c(0.0,0.0);
