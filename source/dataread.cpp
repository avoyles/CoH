...
static char *   readExtractData      (int);
static int      readFgetOneline      (char *);
static Particle readParticleIdentify (int);
...
static const string table_sec[NWORD_SEC]=
...
int readDwba(char *s, Direct *dir)
{
  ...
  while(1){
    ...
    if(head == "ENDDWBA  :"){
      ...
      if(klev<MAX_DIRECT){
        dir->lev[klev].energy = atof(readExtractData(0));
        dir->lev[klev].spin   = abs(atof(readExtractData(1)));
