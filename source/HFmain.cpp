...
//      Main Hartree-Fock Loop
  ...
  for(int n=0 ; n<sys->max_iteration ; n++){
    ...
    double de = 100.0;
    if(n > 0){
      de  = abs(e0.kinetic   /e1.kinetic   - 1.0);
      de += abs(e0.volume    /e1.volume    - 1.0);
      de += abs(e0.surface   /e1.surface   - 1.0);
      de += abs(e0.spinorbit /e1.spinorbit - 1.0);
      de += abs(e0.coulomb   /e1.coulomb   - 1.0);
    }
  ...
  if(!converge){ cerr << "max iteration reached" << endl; }
...
void HFAllocateMemory(const int nr, const int nz, Basis *basis)
{
  ...
  try{
    ...
    message << "MFT memory allocation error for Vector data";
