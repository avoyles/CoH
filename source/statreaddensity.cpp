...
static int  statReadDensityFile (ifstream *, const unsigned int, double *, double **, double **);
static void statInterpolateDensity (const int, double *, double **, double **, double *, Parity **);

static inline double readData(string s, const int i0, const int i1)
{
  if(s.length() <= (unsigned)(i0+i1)) return(0);
  string d = s.substr(i0,i1);
  return(atof(&d[0]));
}

static inline double interpol(double x, double x0, double x1, double y0, double y1)
{
...
void statReadLevelDensity(Nucleus *n)
{
  ...
  cout << "# Reading " << str << " for Z = " << n->za.getZ() << "  A = " << n->za.getA() << endl;

  if(statReadDensityFile(&fp,n->za.getA(),x,y0,y1) != 0) cout << "# A = " << n->za.getA() << " not found" << endl;
...
/**********************************************************/
int statReadDensityFile(ifstream *fp, const unsigned int anum, double *x, double **y0, double **y1)
{
  const int d0 = 42;
  const int dw =  9;
...
void statInterpolateDensity(const int ntotal, double *x, double **y0, double **y1, double *ex, Parity **density)
{
  ...
  for(int k0=0 ; k0<ntotal ; k0++){
    ...
    cout << setw(12) << r;
    for(int j=0 ; j<=3 ; j++){
      cout << setw(12) << density[k0][j].even;
      cout << setw(12) << density[k0][j].odd;
    }
    cout << endl;
  }
  cout << endl;
  ...
  for(int k1=0 ; k1<nefile ; k1++){
    ...
    cout << setw(12) << r;
    for(int j=0 ; j<=3 ; j++){
      cout << setw(12) << y0[k1][j];
      cout << setw(12) << y1[k1][j];
    }
    cout << endl;
  }
  cout << endl;
