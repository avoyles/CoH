...
static void            ccDeformedRadius            (Optical *, Optical *, double);
static double          ccPotentialRadialCoulombRot (int, double, Optical *);
static double          ccPotentialRadialCoulombVib (int, int, double, Optical *);
static double          ccChargedistRatio           (int, double *);

/**********************************************************/
/*     Deformed Optical Potential Radial Form             */
/*     Rotational Excitation by Legendre Exapnsion        */
/**********************************************************/
void ccPotentialFormRot(int nclev, int bmax, double *beta,
                        int zzprod, Optical *omp, double mu, Potential *pc0, Potential **pc1)
{
...
/*
  for(int n=0 ; n<nclev ; n++){
    for(int i=0 ; i<=pc0[n].n_match ; i++){
      cout << setw(12) << (double)i * pc0[n].width;
      cout << setw(12) << pc0[n].mean_field[i].real();
      cout << setw(12) << pc0[n].mean_field[i].imag();

      for(int l=1 ; l<MAX_LAMBDA ; l++){
        cout << setw(12) << pc1[n][l].mean_field[i].real();
        cout << setw(12) << pc1[n][l].mean_field[i].imag();
      }
      cout << endl;
    }
    cout << endl;
...
/**********************************************************/
void ccPotentialFormVib(int nclev, int zzprod, Optical *omp, double mu,
                        Potential *pc0, Potential **pc1, Potential **pc2)
{
  double c1 = 2.0*mu*AMUNIT/VLIGHTSQ/HBARSQ;
  double c2 = c1*CSPO;
  ...
  for(int n=0 ; n<nclev ; n++){
    for(int i=0 ; i<=pc0[n].n_match ; i++){
      ...
      complex<double> v1 = c1 * omPotentialRadialFirstDeriv(r,&omp[n]);
      complex<double> v2 = c1 * omPotentialRadialSecondDeriv(r,&omp[n]);
  ...
  for(int n=0 ; n<nclev ; n++){
    for(int i=0 ; i<=pc0[n].n_match ; i++){
      cout << setw(11) << (double)i * pc0[n].width;
      cout << setw(11) << pc0[n].mean_field[i].real();
      cout << setw(11) << pc0[n].mean_field[i].imag();
      
      for(int l=0 ; l<MAX_LAMBDA ; l++){
        cout << setw(11) << pc1[n][l].mean_field[i].real();
        cout << setw(11) << pc1[n][l].mean_field[i].imag();

        cout << setw(11) << pc2[n][l].mean_field[i].real();
        cout << setw(11) << pc2[n][l].mean_field[i].imag();
      }
      cout << endl;
    }
    cout << endl;
...
/**********************************************************/
double ccChargedistRatio(int bmax, double *beta)
{
...
/**********************************************************/
void ccDeformedRadius(Optical *dom, Optical *omp, double rx)
{
...
/*     ORNL-3240 (1962)                                   */
/**********************************************************/
double ccPotentialRadialCoulombRot(int l, double r, Optical *omp)
{
  double vc = 0.0;
...
double ccPotentialRadialCoulombVib(int n, int l, double r, Optical *omp)
{
  double vc = 0.0;
