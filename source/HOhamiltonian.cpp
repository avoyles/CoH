...
static inline double HOweight(double, double, double);
...
/*
      double t= 0.0;
      if( (s1 == s2) && (l1 == l2) ){
        double nperp = 2*nr2 + l2;
        if( (nr1==nr2) && (nz1 == nz2) ){
          t += (nperp+1.0)*basis->bp2 + (nz2 + 0.5)*basis->bz2;
        }
        else if( (nr1-1 == nr2) && (nz1 == nz2) ){
          t += sqrt((double)nr1*(nr1+l1))*basis->bp2;
        }
        else if( (nr1 == nr2) && (nz1-2 == nz2) ){
          t += -0.5*sqrt((nz1-1.0)*nz1)*basis->bz2;
        }
        t *= pot->effmass[0][1];
      }
      hkin = t;
...
static inline double HOweight(double w, double x, double p)
{
  double z = 0.0;
  /*** avoid 0^0 */
  if( (x == 0.0) && (p == 0.0) ){
