...
/*
          cout << setw(3) << n << setw(3) << k << setw(3) << l << setw(3) << j2;
          cout << setw(12) << col->lev[k].wave_number << setw(5) << cdt[n].open << endl;
*/
          n++;
        }
      }
...
void ccMatrixElementRot(const int k2, Collective *col)
{
  for(int i=0 ; i<col->nlevel ; i++){
    int si2 = (int)(2.0*abs(col->lev[i].spin));
    double yi = sqrt(si2 + 1.0);

    for(int j=0 ; j<=i ; j++){
      int sj2 = (int)(2.0*abs(col->lev[j].spin));
...
/*
  for(int l=0 ; l<MAX_LAMBDA ; l++){
    for(int i=0 ; i<col->nlevel ; i++){
      for(int j=0 ; j<=i ; j++){
        cout << setw(12) << col->qmatrix[l][i*(i+1)/2+j];
      }
      cout << endl;
    }
  }
...
void ccMatrixElementVib(Collective *col)
{
  ...
  for(int l=0 ; l<MAX_LAMBDA ; l++){
    for(int i=0 ; i<col->nlevel ; i++){
      for(int j=0 ; j<=i ; j++){
        cout << setw(3) << col->pindex[i*(i+1)/2+j];
        cout << setw(12) << col->qmatrix[l][i*(i+1)/2+j];
      }
      cout << endl;
    }
...
/*      Coupling Potential                                */
/**********************************************************/
void ccCouplingPotential(const NuclearModel model, const int m, const int jj,
                         double **cv, complex<double> **Vpot, CCdata *cdt,
                         Collective *col, Potential *pc0, Potential **pc1, Potential **pc2)
{
  complex<double> vi(0.0,0.0), vj(0.0,0.0);
  ...
  for(int k=0 ; k<=pc0[0].n_match ; k++){
    for(int i=0 ; i<m ; i++){    int ni = cdt[i].level;
      for(int j=0 ; j<=i ; j++){ int nj = cdt[j].level;
        ...
        Vpot[k][id1] = complex<double>(0.0,0.0);
  ...
  for(int i=0 ; i<m ; i++){    int ni = cdt[i].level;
    for(int j=0 ; j<=i ; j++){ int nj = cdt[j].level; 
      ...
      for(int l=0 ; l<MAX_LAMBDA ; l++){
        ...
        //        cout << setw(3) << jj << setw(3) << i << setw(3) << j << setw(3) <<l;
        //        cout << setw(12) << col->qmatrix[l][id2] << setw(12) << a;
        //        cout << setw(12) << cv[l][id1] << endl;
      }
    }
  }
/*
  for(int l=0 ; l<MAX_LAMBDA ; l++){
    cout << setw(3) << jj << setw(3) << l << endl;
    for(int i=0 ; i<m ; i++){
      for(int j=0 ; j<=i ; j++){
        cout << setw(12)<< cv[l][i*(i+1)/2 + j];
      }
      cout << endl;
    }
  }
...
/**********************************************************/
int ccExternalFunction(double rm, Wavefunc *wfn, Collective *col)
{
...
/**********************************************************/
int ccFindTargetLevel(double ex, Collective *col)
{
  int  kt = -1;
  for(int k=0 ; k<col->nlevel ; k++){
